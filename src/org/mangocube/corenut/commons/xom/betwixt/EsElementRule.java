 ...
 * @since 1.0
 */
class EsElementRule extends EsMappedPropertyRule {
    ...
    public void begin(String name, String namespace, Attributes attributes) throws SAXException {
        ...
        if (implementationClass != null) {
            try {
                Class clazz = Class.forName(implementationClass, true, ClassLoaderResolver.getClassLoader());
     ...
     * @param forceAccessible   if true and updateMethodName is not null, then non-public
    ...
    protected void configureDescriptor(ElementDescriptor elementDescriptor,
        ...
        if (beanClass != null) {
            ...
            if (descriptor == null) {
                ...
                if (log.isWarnEnabled()) log.warn("Cannot find property matching " + name);
            } else {
                //****************************** MangoCube Modification **************************************//
                Class type = elementDescriptor.getPropertyType();
                if (name.indexOf('.') > 0 && type != null) {//Nested Property
                    elementDescriptor.setUpdater(new EsPropertyUpdater(name, type));
                    if (getXMLIntrospector().isPrimitiveType(type)) {
                        elementDescriptor.setTextExpression(new EsPropertyExpression(name));
                    } else {
                        elementDescriptor.setContextExpression(new EsPropertyExpression(name));
                    }
                }
                //***************************************************************************************//
                else {
     ...
     * @param updateMethodName   the name of the custom updater method to user. If null, then
     *                           then
     * @param forceAccessible    if true and updateMethodName is not null, then non-public
     *                           methods will be searched and made accessible
     *                           (Method.setAccessible(true))
    ...
    protected void configureProperty
            (ElementDescriptor elementDescriptor, PropertyDescriptor propertyDescriptor, String updateMethodName,
        ...
        if (getXMLIntrospector().isPrimitiveType(type) || type.isEnum()) {//The enum type is also treated as PrimitiveType
            elementDescriptor.setTextExpression(methodExpression);
        } else if (getXMLIntrospector().isLoopType(type)) {
            log.trace("Loop type ??");

            // don't wrap this in an extra element as its specified in the XML descriptor so no need.
            Expression expression = methodExpression;

            // Support collectives with standard property setters (not adders) that use polymorphism to read objects.
